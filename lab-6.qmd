...
# Check if y and other variables are numeric
  if (sum(!sapply(df_filtered, FUN = is.numeric)) > 0) {
    stop("All columns must be a numeric variable")
  }
...
impute_missing <- function(df, y, ..., impute_fun = mean) {
  ...
  # Filter df
  df_filtered <- df |>
    # keep specified columns (should all be numeric variables)
    dplyr::select({{ y }}, !!!vars)
...
# The function should take a data frame, the formula for the model, and the impute and remove_outliers arguments
...
fit_model <- function(df, mod_formula, remove_outliers = FALSE, impute_missing = FALSE, ...) {
  ...
  # Allow mod_formula to be a formula object
  df |> 
